...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='GET'){
      ...
      const [athlete]=await rows(`athletes?select=id,user_id,date_of_birth,primary_event,secondary_event,selected_events,track_training_years,experience_level,program_start_date,training_goal,created_at&id=eq.${encodeURIComponent(athleteId)}&limit=1`,c.token);
      if(!athlete)return res.status(404).json({error:'Athlete not found'});
      const [profiles,states,prs,notes,strengthMaxes,assignedCheckins,sponsorship]=await Promise.all([
