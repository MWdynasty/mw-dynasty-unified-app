...
-- Applied to Supabase project keqgunlfwhjgcsurynef as 20260920131239 permit_audited_auto_verified_coach_provisioning
...
begin
  ...
  if v_application_id is not null and v_email<>'' then
    select exists(
      ...
      left join public.profiles reviewer on reviewer.user_id=ca.reviewed_by
      where ca.id=v_application_id
        ...
        and (
          (
            ca.verification_status='auto_approved'
            and ca.decision_mode='automatic'
            and ca.verification_checked_at is not null
          )
          or
          (
            ca.reviewed_by is not null
