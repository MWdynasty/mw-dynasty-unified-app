...
async function boot(){
  ...
  try{
    const ur=await fetch(SUPABASE_URL+'/auth/v1/user',{headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+session.access_token}});if(!ur.ok)throw new Error('session expired');
    const user=await ur.json();session.user=user;window.MW_SESSION=session;const uid=user.id;
    ...
    if(!account||account.account_status!=='active'){localStorage.removeItem(SESSION_KEY);location.replace('/?access=membership');return}
