...
function saveSession(session){if(session?.access_token)localStorage.setItem(SESSION_KEY,JSON.stringify(session))}
function clearAndReturnToAthlete(){localStorage.removeItem(SESSION_KEY);location.replace('/athlete/')}
...
async function boot(){
  ...
  if(!session?.access_token){
    ...
    if(!session){clearAndReturnToAthlete();return}
  }
  try{
