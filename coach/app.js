...
async function practiceModePage(){
  ...
  let rep=1,repStart=0,repStarted=false,tick=null,activeGroup='all',athletes=[],repResults=[],sessionResults=[];
  ...
  startBtn.onclick=()=>{if(repStart){stopTick();repStart=0;startBtn.textContent='RESUME REP';clockState.textContent=`Rep ${rep} paused — resume or move to the next rep`;return}repStarted=true;repStart=performance.now();startBtn.textContent='PAUSE';clockState.textContent=`Rep ${rep} running — tap athletes at the line`;loop()};
  resetBtn.onclick=()=>{stopTick();repStart=0;repStarted=false;repResults=[];paintClock();startBtn.textContent='START REP';clockState.textContent=`Ready for Rep ${rep}`;timingState.textContent='';renderTiming()};
  ...
  nextBtn.onclick=()=>{if(!repStarted&&!repResults.length){timingState.textContent=`Start Rep ${rep} first.`;toast(`Start Rep ${rep} first`);return}commitRep();rep++;repResults=[];repStarted=false;stopTick();repStart=0;paintClock();repLabel.textContent=`REP ${rep}`;clockState.textContent=`Ready for Rep ${rep}`;startBtn.textContent='START REP';timingState.textContent='';renderTiming()};
  finishBtn.onclick=async()=>{
    if(repResults.length)commitRep();
    stopTick();repStart=0;repStarted=false;
    if(!sessionResults.length){
      repResults=[];rep=1;repLabel.textContent='REP 1';clockState.textContent='Ready for Rep 1';startBtn.textContent='START REP';paintClock();renderTiming();
      timingState.innerHTML='<b>Session finished.</b> No athlete finish times were recorded, so there was nothing to save.';
      toast('Practice session finished');
      return;
    }
    ...
    try{
      ...
      finishBtn.textContent='FINISH & SAVE';finishBtn.disabled=false;
    }catch(e){
      timingState.innerHTML=`<b>Save error:</b> ${escapeHtml(e.message)}`;
      finishBtn.disabled=false;finishBtn.textContent='FINISH & SAVE';
    }
  };
