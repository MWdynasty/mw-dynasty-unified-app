...
async function seasonPage(){
  if(experience==='core'){
    pageBase('Season Planner','Smart season planning is available with Coach Intelligence and MW Sprint Performance.',`
      <div class="tile"><span class="status-kicker">COACH CORE</span><h3>Basic calendar included</h3><p>Coach Core can schedule practices, meets, testing, and team events. Upgrade to Coach Intelligence for school-break and exam-week awareness, or MW Sprint Performance for those constraints integrated with the 41-week MW sprint + strength system.</p><button class="action" data-page="calendar" style="margin-top:12px">Open Core Calendar</button><button class="back" data-page="account" style="margin-top:12px">View Membership</button></div>`);
    bindPageNavigation(app);return;
  }
  const rec=mwLoad('seasonRecommendation',null),performance=experience==='performance';
  pageBase('AI Season Planner',performance?'Plan around school constraints while preserving the 41-week MW sprint + strength progression.':'Build and optimize your season around school constraints while keeping the coach in control.',`<div class="panel-grid"><div class="tile"><h3>Training-Year Position</h3><p id="seasonPlannerCalendar">Loading MW season calendar…</p></div><div class="tile"><h3>Next Meet</h3><p id="seasonPlannerNextMeet">Loading live meet schedule…</p></div><div class="tile" style="grid-column:1/-1"><h3>${performance?'41-Week Schedule Integration':'School / Schedule Constraints'}</h3><p id="seasonPlannerConstraints">Loading school calendar…</p><button class="back" data-page="calendar" style="margin-top:10px">Open Calendar</button></div></div><div class="form" style="margin-top:14px"><label>Season Goal<textarea id="seasonGoal" rows="4" placeholder="What should the team be ready for?"></textarea></label><button class="action" id="genSeason">Generate Recommendation</button><div id="seasonOut" class="tile" style="${rec?'':'display:none;'}">${rec?escapeHtml(rec):''}</div></div>`);
  ...
  document.getElementById('genSeason').onclick=()=>{const goal=document.getElementById('seasonGoal')?.value.trim()||'Protect speed quality while preparing for the next meet.',out=document.getElementById('seasonOut'),cal=coachSeasonCalendar,calendar=cal?coachCalendarSummary(cal):'MW season calendar',meet=nextMeet?` Next meet: ${nextMeet.title} on ${new Date(nextMeet.starts_at).toLocaleDateString()}.`:'',constraintText=constraints.length?` Upcoming constraints: ${constraints.slice(0,4).map(e=>`${e.title} (${coachDateRange(e)}; ${coachConstraintImpactLabel(e.training_impact)})`).join('; ')}.`:'',tierInstruction=performance?' Preserve the current 41-week MW sprint + strength phase intent and shift only what is necessary around the saved constraints.':' Use the saved constraints to organize the coach’s own program without inventing or replacing the coach’s training system.';const text=`Recommendation: use ${calendar} as the scheduling reference.${meet}${constraintText} Keep no-practice dates clear and reduce scheduling pressure during reduced-load periods.${tierInstruction} Coach approval is required before changing official program state. Goal: ${goal}`;mwStore('seasonRecommendation',text);if(out){out.style.display='block';out.textContent=text}}
}
