...
let accountAccess={role:null,tier:null,isFounder:false,firstName:'',lastName:''};
...
async function verifyCoachAccess(session){
  ...
  accountAccess={role:d.role||null,tier:d.tier||null,isFounder:!!d.isFounder,firstName:d.firstName||'',lastName:d.lastName||''};
...
async function adjustmentPage(){let status=mwLoad('seasonAdjustment','Pending coach review');pageBase('AI Season Adjustment','Detect → Analyze → Recommend → Coach Approves → System Executes.',`<div class="tile"><h3>Live Athlete Review</h3><div id="adjustLive"><p>Reviewing assigned athlete training and performance signals…</p></div><p><b>Status:</b> <span id="adjustStatus">${escapeHtml(status)}</span></p><div id="adjustActions" style="display:none;gap:10px;flex-wrap:wrap;margin-top:12px"><button class="action" id="approveAdjustment">Approve for Review</button><button class="back" id="dismissAdjustment">Dismiss</button></div></div>`);
  ...
  try{const [roster,perf]=await Promise.all([fetchCoachRoster(),fetchCoachPerformance().catch(()=>({athletes:[]}))]),athletes=roster.athletes||[],pm=new Map((perf.athletes||[]).map(x=>[x.athlete_id,x])),candidates=[];
    ...
    if(pick){recommendation={athleteId:pick.a.id,name:pick.a.name,level:pick.classified.level,reasons:pick.classified.reasons};adjustLive.innerHTML=`<p><b>${escapeHtml(pick.a.name)}</b> · ${escapeHtml(athleteStatusLabel(pick.classified.level))}</p><p>${pick.classified.reasons.map(escapeHtml).join(' · ')}</p><p><small>Recommendation: review this athlete's attendance, recent completion, and performance context before changing progression. MW will not change the athlete's official program state from this screen.</small></p>`;adjustActions.style.display='flex'}else{adjustLive.innerHTML='<p><b>No adjustment signal is active.</b></p><p>Assigned athlete activity and available performance data do not currently produce a review flag.</p>';adjustStatus.textContent='No pending adjustment'}
  }catch(e){adjustLive.innerHTML=`<p><b>Live review unavailable.</b></p><p>${escapeHtml(e.message)}</p>`;adjustStatus.textContent='Could not evaluate live data'}
  const approve=document.getElementById('approveAdjustment'),dismiss=document.getElementById('dismissAdjustment');
  if(approve)approve.onclick=()=>{if(!recommendation)return;status=`Reviewed by coach — ${recommendation.name}; no automatic program change applied`;mwStore('seasonAdjustment',status);adjustStatus.textContent=status};
  if(dismiss)dismiss.onclick=()=>{status='Dismissed by coach';mwStore('seasonAdjustment',status);adjustStatus.textContent=status}
}
