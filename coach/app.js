...
async function practiceModePage(){
  pageBase('Practice Mode','A clean sideline view for running practice without digging through the full dashboard.',`
    <div class="practice-mode-head">
      <div><span class="status-kicker">LIVE PRACTICE</span><h2 id="practiceClock">00:00</h2><small id="practiceClockState">Stopwatch ready</small></div>
      <div class="practice-timer-actions"><button class="action" id="practiceTimerStart">Start</button><button class="back" id="practiceTimerReset">Reset</button></div>
    </div>
    <div class="panel-grid" style="margin-top:14px">
      <button class="tile practice-launch" id="practiceTraining"><h3>🏃 Today’s Training</h3><p>${experience==='performance'?'Open the MW Sprint Performance workout.':'Open your coaching program.'}</p><b>OPEN →</b></button>
      <button class="tile practice-launch" id="practiceCoachMW" ${experience==='core'?'style="display:none"':''}><h3>🧠 Coach MW</h3><p>Ask a quick coaching question without leaving Practice Mode.</p><b>OPEN →</b></button>
    </div>
    <div class="tile" style="margin-top:14px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><div><h3>Quick Attendance</h3><p>Tap each athlete’s status, then save once.</p></div><b id="practiceAttendanceCount">Loading…</b></div><div class="list" id="practiceRoster" style="margin-top:10px"><div class="row">Loading assigned athletes…</div></div><button class="action" id="practiceSaveAttendance" style="margin-top:14px" disabled>Save Practice Attendance</button><div id="practiceAttendanceState" style="margin-top:10px"></div></div>
  `);
  const training=document.getElementById('practiceTraining');if(training)training.onclick=()=>openPage(experience==='performance'?'mwtrack':'programs');
  const ai=document.getElementById('practiceCoachMW');if(ai)ai.onclick=()=>openPage('coachmw');
  let elapsed=Number(sessionStorage.getItem('mwPracticeElapsed')||0),startedAt=Number(sessionStorage.getItem('mwPracticeStartedAt')||0),timer=null;
  const clock=document.getElementById('practiceClock'),clockState=document.getElementById('practiceClockState'),startBtn=document.getElementById('practiceTimerStart');
  const paint=()=>{const total=elapsed+(startedAt?Math.max(0,Math.floor((Date.now()-startedAt)/1000)):0),m=Math.floor(total/60),s=total%60;if(clock)clock.textContent=`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;if(startBtn)startBtn.textContent=startedAt?'Pause':'Start';if(clockState)clockState.textContent=startedAt?'Practice timer running':'Practice timer paused'};
  const run=()=>{if(timer)clearInterval(timer);paint();if(startedAt)timer=setInterval(paint,1000)};run();
  if(startBtn)startBtn.onclick=()=>{if(startedAt){elapsed+=Math.max(0,Math.floor((Date.now()-startedAt)/1000));startedAt=0}else startedAt=Date.now();sessionStorage.setItem('mwPracticeElapsed',String(elapsed));sessionStorage.setItem('mwPracticeStartedAt',String(startedAt));run()};
  const reset=document.getElementById('practiceTimerReset');if(reset)reset.onclick=()=>{elapsed=0;startedAt=0;sessionStorage.removeItem('mwPracticeElapsed');sessionStorage.removeItem('mwPracticeStartedAt');run()};
  ...
  try{const d=await fetchCoachRoster(),athletes=Array.isArray(d.athletes)?d.athletes:[];if(count)count.textContent=`${athletes.length} athlete${athletes.length===1?'':'s'}`;if(!athletes.length){roster.innerHTML='<div class="row">No assigned athletes yet.</div>';return}
    roster.innerHTML=athletes.map(a=>`<div class="row practice-athlete"><span><b>${escapeHtml(a.name)}</b><br><small>${escapeHtml(a.event||'Events not set')}</small></span><select data-practice-athlete="${escapeHtml(a.id)}" aria-label="Practice attendance for ${escapeHtml(a.name)}"><option value="present">Present</option><option value="late">Late</option><option value="excused">Excused</option><option value="absent">Absent</option><option value="injured">Injured</option></select></div>`).join('');save.disabled=false;
    save.onclick=async()=>{const rows=[...document.querySelectorAll('[data-practice-athlete]')];save.disabled=true;save.textContent='Saving…';if(state)state.textContent='Syncing attendance…';try{const token=mwSessionToken(),sessionDate=new Date().toISOString().slice(0,10);await Promise.all(rows.map(async el=>{const r=await fetch('/api/coach/attendance',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify({athleteId:el.dataset.practiceAthlete,status:el.value,sessionDate,sessionType:'training'})});const x=await r.json().catch(()=>({}));if(!r.ok)throw new Error(x.error||'Attendance save failed')}));if(state)state.innerHTML='<b>✓ Practice attendance saved.</b>';toast('Practice attendance saved')}catch(e){if(state)state.innerHTML=`<b>Attendance error:</b> ${escapeHtml(e.message)}`}finally{save.disabled=false;save.textContent='Save Practice Attendance'}};
  }catch(e){roster.innerHTML=`<div class="row">Roster unavailable: ${escapeHtml(e.message)}</div>`;if(count)count.textContent='—'}
}
...
async function openNotifications(){
  try{await window.mwNativePermission?.('notifications')}catch{}
...
async function inviteAthleteModal(){
  ...
  let sponsoredBillingActive=false;try{const b=await coachBillingRequest();sponsoredBillingActive=!!b?.status?.sponsored_billing_active}catch{}
  const modal=mwModal('Invite Athlete',`<div class="tile"><h3>Connect an athlete to your dashboard</h3><p>The athlete can be new to MW Dynasty or already have an account. Choose who is paying before you send the invitation.</p></div><div class="form" style="margin-top:14px"><label>Athlete Email<input id="inviteEmail" type="email" inputmode="email" placeholder="athlete@example.com"></label><label>Invite Type<select id="inviteType"><option value="coach_invite">Coach invitation</option><option value="team_invite">Team invitation</option></select></label><label>Who is paying?<select id="inviteBillingType"><option value="coach_sponsored" ${sponsoredBillingActive?'':'disabled'}>I'm sponsoring this athlete (+&#36;${sponsorPrice}/month)${sponsoredBillingActive?'':' — billing setup required'}</option><option value="self_pay">Athlete pays for their own membership</option></select></label><div class="tile" id="inviteBillingHelp">${sponsoredBillingActive?'<b>Coach Sponsored</b><br><small>The athlete creates or signs into their profile and skips membership checkout. Their MW access is billed to your coach account.</small>':'<b>Athlete Self-Pay</b><br><small>Coach-sponsored billing is not active on this account yet. You can still connect the athlete as self-pay.</small>'}</div><button class="action" id="createInvite">Create Secure Invitation</button><div id="inviteState" class="tile" style="display:none"></div></div>`);
  const billingSelect=modal.querySelector('#inviteBillingType'),billingHelp=modal.querySelector('#inviteBillingHelp');if(!sponsoredBillingActive)billingSelect.value='self_pay';
